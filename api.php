...
function handleSaveConfig() {
    ob_start(); // Capturar cualquier advertencia inesperada
    $body = getJsonBody();
    $clientId = trim($body['client_id'] ?? '');
    $clientSecret = trim($body['client_secret'] ?? '');

    if (empty($clientId) || empty($clientSecret)) {
        jsonError('El Client ID y Secret son obligatorios');
    }

    // Generar contenido de forma segura para evitar errores de sintaxis
    $content = "<?php\n" .
               "// Autogenerated by SpotiDown\n" .
               "if (!defined('SPOTIPY_CLIENT_ID')) define('SPOTIPY_CLIENT_ID', '" . addslashes($clientId) . "');\n" .
               "if (!defined('SPOTIPY_CLIENT_SECRET')) define('SPOTIPY_CLIENT_SECRET', '" . addslashes($clientSecret) . "');\n" .
               "if (!defined('DEFAULT_AUDIO_PROVIDER')) define('DEFAULT_AUDIO_PROVIDER', 'piped');\n";
    ...
    if ($success !== false) {
        ...
        jsonError('Error: No se pudo escribir el archivo .env.php. Revisa que la carpeta spotidown tenga permisos de escritura.');
    }
}
